...
const newRender = `// --- ASCII ART RENDERING (floating top‑right) ---
const renderASCII = () => {
    // Create a fixed‑position container in the top‑right corner
    const container = document.createElement('div');
    container.id = 'ascii-floating';
    // Size – 300 px square (adjustable)
    const size = 300;
    ...
    img.onload = () => {
        ...
        // Adjust off‑screen canvas for ASCII conversion
        const cols = 150;
...
// Replace the old ASCII block (from "const renderASCII" to the following "renderASCII();")
html = html.replace(/const renderASCII[\s\S]*?renderASCII\(\);/, newRender);
fs.writeFileSync('index.html', html);
console.log('ASCII floating version applied.');
