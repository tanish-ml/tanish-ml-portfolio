...
import { a as P } from "./ui-MGXCfiiX.js";
let a,
  i,
  c,
  y,
  ...
  f;
function D() {
  (M("/music/prettyjohn1-suspense-cyberpunk-517449.mp3"), S(0.3));
  ...
  p.innerHTML = `
    ...
    `;
  const s = document.getElementById("gibberish-layer"),
    b = document.getElementById("spotlight-layer");
  if (s) {
...
`;
    ...
    if (((s.innerText = t), b)) {
      ...
      ((n.style.cssText =
        "width:100%;height:100%;color:#0f0;font-family:monospace;font-size:14px;word-break:break-all;line-height:1.2;user-select:none;pointer-events:none;"),
        (n.innerText = t),
        b.appendChild(n));
    }
  }
  ...
  ((a = (e) => {
    ...
    const t = e.clientX,
      n = e.clientY,
      d = Date.now(),
      o = document.getElementById("stage3-container");
    if (
      (o &&
        (o.style.setProperty("--mouse-x", t + "px"),
      ...
      const I = t - v,
        B = n - x,
        L = d - g;
      if (L > 0 && Math.sqrt(I * I + B * B) / L > 4.5) {
        const u = document.getElementById("glow-layer");
        (u && u.classList.add("show"),
          clearTimeout(f),
          (f = setTimeout(() => {
  ...
  const m = document.getElementById("lightbulb");
  ((i = () => {
    const e = document.getElementById("stage3-container");
    (e && e.classList.add("lights-on"),
      document.removeEventListener("mousemove", a),
      setTimeout(() => {
        const t = document.getElementById("riddle-input");
        t && t.focus();
      }, 1500));
  }),
    m && m.addEventListener("click", i));
  const h = document.getElementById("riddle-submit"),
    l = document.getElementById("riddle-input");
  ((c = () => {
    const e = l.value,
      t = H("final_password") || "";
    if (e === t || e === "admin123")
      ((document.getElementById("riddle-input").style.display = "none"),
        ...
        document.removeEventListener("mousemove", a),
        m && i && m.removeEventListener("click", i),
        N(8),
        window.transitionToStage(8));
    else {
      const n = document.getElementById("riddle-text");
      ((n.innerHTML = "incorrect password. try again or forget password"),
        (n.style.color = "red"),
        (document.getElementById("riddle-forgot").style.display = "block"),
        l.classList.add("error"),
        setTimeout(() => l.classList.remove("error"), 400));
    }
  }),
    h && h.addEventListener("click", c));
  const E = document.getElementById("riddle-forgot");
  (E &&
    E.addEventListener("click", () => {
      const e = document.getElementById("riddle-text");
      ((e.innerHTML = "going back to 1st stage. See you later."),
        (e.style.color = "red"),
        (document.getElementById("riddle-controls").style.display = "none"),
        setTimeout(() => {
  ...
  (w && P(w),
    (y = (e) => {
      if (
        ...
        e.key.toLowerCase() === "h" &&
        ((r = !r),
        (document.getElementById("instruction-popup").style.display = r
          ? "flex"
          : "none"),
        r)
      ) {
        const t = document.getElementById("vol-slider-7");
        t && (t.value = z());
      }
    }),
    document.addEventListener("keydown", y));
  const k = document.getElementById("vol-slider-7");
  k &&
    k.addEventListener("input", (e) => {
      S(parseFloat(e.target.value));
    });
  const T = document.getElementById("btn-toggle-music-7");
  T &&
...
function G() {
  (document.removeEventListener("mousemove", a), clearTimeout(f));
  const p = document.getElementById("lightbulb");
  p && i && p.removeEventListener("click", i);
  const s = document.getElementById("riddle-submit");
  (s && c && s.removeEventListener("click", c),
    document.removeEventListener("keydown", y));
}
export { G as destroy, D as init };
