...
import { a as x } from "./ui-MGXCfiiX.js";
let m,
  h,
  r,
  g,
...
const p = [
  ...
  },
  {
    ...
    check: (e) =>
      e.split("").reduce((t, i) => (/\d/.test(i) ? t + parseInt(i) : t), 0) ===
      35,
  },
  {
    id: 4,
  ...
  },
  {
    ...
    check: (e) => /[IVXLCDM]/.test(e),
  },
  {
    id: 6,
    text: "Must contain the name of the internship where you met.",
    check: (e) => e.toLowerCase().includes("dentsu"),
  },
  {
    id: 7,
  ...
  },
  {
    ...
    check: (e) => e.length % 2 === 0,
  },
  {
    id: 10,
    text: "Must contain the answer to 'Who is coolest?'.",
    check: (e) => e.toLowerCase().includes("me"),
  },
  {
    id: 11,
...
function N() {
  (L(
    "/music/hauntsync-dark-orchestral-synthwave-tribute-to-drone-operators-231877.mp3",
  ),
    b(0.3));
  ...
  ((e.innerHTML = `
    ...
    `),
    (m = document.getElementById("login-form")),
    (h = document.getElementById("password-game")),
    (r = document.getElementById("inp-password")),
    (g = document.getElementById("rules-container")),
    (w = document.getElementById("char-count")),
    k() && document.getElementById("btn-dev-skip").classList.remove("hidden"),
    document.getElementById("btn-dev-skip").addEventListener("click", () => {
      E();
    }),
    document.getElementById("btn-final-submit").addEventListener("click", E),
    document.getElementById("inp-height").addEventListener("input", (n) => {
      document.getElementById("err-height").innerText =
        "Dev doesn't know your height so we'll trust you.";
    }),
    document.getElementById("btn-verify").addEventListener("click", T),
    r.addEventListener("input", () => {
      ((w.innerText = `Length: ${r.value.length}`), u());
    }),
    e.querySelectorAll(".input-group label").forEach((n) => {
      x(n);
    }));
  const t = document.querySelector("#instruction-popup h2");
  (t && x(t),
    (f = (n) => {
      if (
        !(n.target.tagName === "INPUT" || n.target.tagName === "TEXTAREA") &&
        n.key.toLowerCase() === "h" &&
        ((c = !c),
        ...
        const l = document.getElementById("vol-slider-1");
        l && (l.value = I());
      }
    }),
    document.addEventListener("keydown", f));
  const i = document.getElementById("vol-slider-1");
  i &&
    i.addEventListener("input", (n) => {
      b(parseFloat(n.target.value));
    });
  const o = document.getElementById("btn-toggle-music-1");
  o &&
...
function T() {
  ...
  const t = (o, n, l) => {
    const y = document.getElementById(`err-${o}`),
      v = document.getElementById(`inp-${o}`);
    n
      ? (y.innerText = "")
      : ((y.innerText = l),
        v.parentElement.classList.add("shake"),
        setTimeout(() => v.parentElement.classList.remove("shake"), 400),
        (s = !1));
  };
  (t("firstname", e.first === "bhanu", "Incorrect first name."),
    t("lastname", e.last === "girotra", "Incorrect last name."),
    t(
      "birthday",
      ["19th aug", "19 aug", "19th august", "19 august"].includes(e.bday),
      "Incorrect birthday.",
    ),
    t("internship", e.intern === "dentsu", "Incorrect internship."),
    t("coolest", e.cool === "me", "Incorrect."),
    t("currency", e.curr === "egg" || e.curr === "an egg", "Incorrect."),
    t("trapped", e.trap === "me", "Incorrect."),
    s &&
      (a("firstname", e.first),
      a("lastname", e.last),
      a("birthday", e.bday),
...
function u() {
  ...
  for (let i = 0; i < d; i++) {
    const o = p[i],
      n = o.check(e);
    (s.push({ rule: o, passed: n }), n || (t = !1));
  }
  if (
    (s.sort((i, o) =>
      i.passed === o.passed ? o.rule.id - i.rule.id : i.passed ? 1 : -1,
    ),
    (g.innerHTML = ""),
    s.forEach((i) => {
      const o = i.rule,
  ...
  t && d === p.length
    ? document
        .getElementById("final-success-container")
...
function E() {
  ((r.disabled = !0), a("final_password", r.value), C(2));
  const e = document.createElement("div");
  ((e.className = "unlock-overlay"),
    (e.innerText = "ACCESS GRANTED"),
    document.body.appendChild(e),
    setTimeout(() => {
      (e.parentElement && e.remove(), window.transitionToStage(2));
    }, 1500));
}
function P() {
  (document.querySelectorAll(".unlock-overlay").forEach((t) => t.remove()),
    document.removeEventListener("keydown", f));
  const s = document.getElementById("app");
  s.innerHTML = "";
}
export { P as destroy, N as init };
